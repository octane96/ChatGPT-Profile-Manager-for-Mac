...
- メニューバー：ログイン時起動、コンパクト表示、25%/10%閾値通知 / Menu Bar: launch at login, compact status, and 25%/10% threshold notifications
